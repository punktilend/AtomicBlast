...
private fun PreviewShell(activeRoute: String, content: @Composable () -> Unit) {
    ...
    CompositionLocalProvider(LocalAtomicBlastColors provides c) {
        AtomicBlastTheme {
            Scaffold(
                containerColor = c.bg,
                bottomBar = {
    ...
    PreviewShell("dashboard") {
        Column(
            ...
            Row(verticalAlignment = Alignment.CenterVertically) {
                Text("AtomicBlast", fontSize = 28.sp, fontWeight = FontWeight.Bold, color = c.green)
